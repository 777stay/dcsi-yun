...
class CppJson:
    def __init__(self,droneNo,scanningDensity,polygon,obstacles,pathsStrictlyInPoly,initialPos,rPortions):
...
if len(sys.argv)>1:
    ...
    # d:垂直扩展 l:线性扩展
    ka.kml2area_jwh(40,20,int(sys.argv[1]))
    # cj = CppJson(int(sys.argv[1]), cm.Rd / 2, ka.polygon, ka.obstacles, False, ka.initialPos, ka.rPortions)
    cj=CppJson(int(sys.argv[1]),20,ka.polygon,ka.obstacles,False,ka.initialPos,ka.rPortions)
    cj.py2json()